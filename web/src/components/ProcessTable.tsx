...
export function ProcessTable({ gpus }: { gpus: GpuSnapshot[] }) {
  ...
  return (
    <div className="pt-1">
      <div
        className="grid grid-cols-[2rem_4rem_4.5rem_minmax(6rem,1fr)_4.5rem] gap-x-2 px-2 py-1 text-[9px] font-semibold uppercase tracking-wide"
      ...
      {rows.length === 0 ? (
        ...
        rows.map(({ gpu, proc }) => (
          <div
            ...
            className="grid grid-cols-[2rem_4rem_4.5rem_minmax(6rem,1fr)_4.5rem] items-baseline gap-x-2 px-2 py-0.5 text-[11px] leading-4"
          >
